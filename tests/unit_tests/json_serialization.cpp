...
namespace
{
    ...
    //! Serialize `value` and parse the result back into `doc`, without deserializing it.
    template<typename T>
    void to_json_document(const T& value, rapidjson::Document& doc)
    {
      epee::byte_stream buffer;
      {
        rapidjson::Writer<epee::byte_stream> dest{buffer};
    ...
    //! \return A `crypto::hash` from 64 hex characters, for distinguishable test values.
    crypto::hash hash_from_hex(const boost::string_ref hex)
    {
      crypto::hash out{};
      if (!epee::string_tools::hex_to_pod(hex, out))
      {
        throw std::logic_error{"test hash literal is not 32 bytes of hex"};
      }
      return out;
    }
...
TEST(JsonSerialization, BlockHeaderResponse)
{
  cryptonote::rpc::BlockHeaderResponse header{};
  header.major_version = 16;
  header.minor_version = 16;
  header.timestamp = 1712345678;
  header.prev_id = hash_from_hex("2f8d1b0c4e6a7593ac1de2f30415263748596a7b8c9d0e1f2031425364758697");
  header.nonce = 0xfeed1234;
  header.height = 3100001;
  header.depth = 12;
  header.hash = hash_from_hex("9a8b7c6d5e4f30211f2e3d4c5b6a798807162534435261708f9eadbc0d1e2f30");
  // Genuinely wider than 64 bits: the low 64 bits are `difficulty` and the
  // high 64 bits (5421) are only recoverable through `difficulty_top64`.
  header.wide_difficulty = cryptonote::difficulty_type{"100000000000000000005443"};
  header.difficulty = 200376420520695107;
  header.reward = 600000000000;

  const auto header_copy = test_json(header);

  EXPECT_EQ(header.major_version, header_copy.major_version);
  EXPECT_EQ(header.minor_version, header_copy.minor_version);
  EXPECT_EQ(header.timestamp, header_copy.timestamp);
  EXPECT_EQ(header.prev_id, header_copy.prev_id);
  EXPECT_EQ(header.nonce, header_copy.nonce);
  EXPECT_EQ(header.height, header_copy.height);
  EXPECT_EQ(header.depth, header_copy.depth);
  EXPECT_EQ(header.hash, header_copy.hash);
  EXPECT_EQ(header.wide_difficulty, header_copy.wide_difficulty);
  EXPECT_EQ(header.difficulty, header_copy.difficulty);
  EXPECT_EQ(header.reward, header_copy.reward);

  // The high half must survive on its own, not merely as part of the whole.
  EXPECT_EQ(std::uint64_t(5421), (header_copy.wide_difficulty >> 64).convert_to<std::uint64_t>());

  rapidjson::Document doc;
  ASSERT_NO_THROW(to_json_document(header, doc));
  ASSERT_TRUE(doc.HasMember("difficulty"));
  ASSERT_TRUE(doc.HasMember("difficulty_top64"));
  ASSERT_TRUE(doc["difficulty"].IsUint64());
  ASSERT_TRUE(doc["difficulty_top64"].IsUint64());
  EXPECT_EQ(header.difficulty, doc["difficulty"].GetUint64());
  EXPECT_EQ(std::uint64_t(5421), doc["difficulty_top64"].GetUint64());
}

TEST(JsonSerialization, BlockHeaderResponseWithoutDifficultyTop64)
{
  cryptonote::rpc::BlockHeaderResponse header{};
  header.major_version = 16;
  header.minor_version = 16;
  ...
  // A daemon that predates `difficulty_top64` emits every other key; parsing
  // its payload must succeed and yield the low 64 bits it did carry.
  rapidjson::Document doc;
  ASSERT_NO_THROW(to_json_document(header, doc));
  ASSERT_TRUE(doc.RemoveMember("difficulty_top64"));
  ASSERT_FALSE(doc.HasMember("difficulty_top64"));

  cryptonote::rpc::BlockHeaderResponse header_copy{};
  ASSERT_NO_THROW(cryptonote::json::fromJsonValue(doc, header_copy));

  EXPECT_EQ(cryptonote::difficulty_type(header.difficulty), header_copy.wide_difficulty);
  EXPECT_EQ(header.difficulty, header_copy.difficulty);
  EXPECT_EQ(header.height, header_copy.height);
  EXPECT_EQ(header.hash, header_copy.hash);
  EXPECT_EQ(header.prev_id, header_copy.prev_id);
...
TEST(JsonSerialization, TxInPool)
{
  ...
  cryptonote::rpc::tx_in_pool pool_tx{};
  pool_tx.tx = test::make_miner_transaction(acct.get_keys().m_account_address);

  crypto::hash tx_hash{};
  ASSERT_TRUE(cryptonote::get_transaction_hash(pool_tx.tx, tx_hash));
  ASSERT_NE(crypto::hash{}, tx_hash);

  pool_tx.tx_hash = tx_hash;
  pool_tx.blob_size = 1234;
  pool_tx.weight = 1300;
  pool_tx.fee = 98765432;
  pool_tx.max_used_block_hash = hash_from_hex("00112233445566778899aabbccddeeff102132435465768798a9bacbdcedfe0f");
  pool_tx.max_used_block_height = 2999999;
  pool_tx.kept_by_block = true;
  pool_tx.last_failed_block_hash = hash_from_hex("ffeeddccbbaa99887766554433221100f0edcbab9a897867564534231201f0e0");
  pool_tx.last_failed_block_height = 2999998;
  pool_tx.receive_time = 1712345679;
  pool_tx.last_relayed_time = 1712345680;
  pool_tx.relayed = true;
  pool_tx.do_not_relay = false;
  pool_tx.double_spend_seen = true;

  const auto pool_tx_copy = test_json(pool_tx);

  crypto::hash tx_copy_hash{};
  ASSERT_TRUE(cryptonote::get_transaction_hash(pool_tx_copy.tx, tx_copy_hash));
  EXPECT_EQ(tx_hash, tx_copy_hash);

  EXPECT_EQ(pool_tx.tx_hash, pool_tx_copy.tx_hash);
  EXPECT_EQ(pool_tx.blob_size, pool_tx_copy.blob_size);
  EXPECT_EQ(pool_tx.weight, pool_tx_copy.weight);
  EXPECT_EQ(pool_tx.fee, pool_tx_copy.fee);
  EXPECT_EQ(pool_tx.max_used_block_hash, pool_tx_copy.max_used_block_hash);
  EXPECT_EQ(pool_tx.max_used_block_height, pool_tx_copy.max_used_block_height);
  EXPECT_EQ(pool_tx.kept_by_block, pool_tx_copy.kept_by_block);
  EXPECT_EQ(pool_tx.last_failed_block_hash, pool_tx_copy.last_failed_block_hash);
  EXPECT_EQ(pool_tx.last_failed_block_height, pool_tx_copy.last_failed_block_height);
  EXPECT_EQ(pool_tx.receive_time, pool_tx_copy.receive_time);
  EXPECT_EQ(pool_tx.last_relayed_time, pool_tx_copy.last_relayed_time);
  EXPECT_EQ(pool_tx.relayed, pool_tx_copy.relayed);
  EXPECT_EQ(pool_tx.do_not_relay, pool_tx_copy.do_not_relay);
  EXPECT_EQ(pool_tx.double_spend_seen, pool_tx_copy.double_spend_seen);

  // `tx_hash` has always been written, so it is a required read like every
  // other member here: its absence is a malformed payload, not an older one.
  rapidjson::Document doc;
  ASSERT_NO_THROW(to_json_document(pool_tx, doc));
  ASSERT_TRUE(doc.RemoveMember("tx_hash"));
  cryptonote::rpc::tx_in_pool incomplete{};
  EXPECT_THROW(cryptonote::json::fromJsonValue(doc, incomplete), cryptonote::json::MISSING_KEY);
}

TEST(JsonSerialization, OutputDistributionCumulative)
{
  cryptonote::rpc::output_distribution dist{};
  dist.data.distribution = {1, 5, 14, 30, 55};
  dist.data.start_height = 2000000;
  dist.data.base = 4242;
  dist.amount = 1000000000000;
  dist.cumulative = true;

  const auto dist_copy = test_json(dist);

  EXPECT_EQ(dist.data.distribution, dist_copy.data.distribution);
  EXPECT_EQ(dist.data.start_height, dist_copy.data.start_height);
  EXPECT_EQ(dist.data.base, dist_copy.data.base);
  EXPECT_EQ(dist.amount, dist_copy.amount);
  EXPECT_TRUE(dist_copy.cumulative);

  rapidjson::Document doc;
  ASSERT_NO_THROW(to_json_document(dist, doc));
  ASSERT_TRUE(doc.HasMember("cumulative"));
  ASSERT_TRUE(doc["cumulative"].IsBool());
  EXPECT_TRUE(doc["cumulative"].GetBool());
}

TEST(JsonSerialization, OutputDistributionNonCumulative)
{
  cryptonote::rpc::output_distribution dist{};
  dist.data.distribution = {1, 4, 9, 16, 25};
  dist.data.start_height = 2000001;
  dist.data.base = 0;
  dist.amount = 0;
  dist.cumulative = false;
  ...
  EXPECT_EQ(dist.data.base, dist_copy.data.base);
  EXPECT_EQ(dist.amount, dist_copy.amount);
  EXPECT_FALSE(dist_copy.cumulative);

  // The flag must be written as an explicit `false`, so that a false result
  // cannot be an artefact of the destination's default value.
  rapidjson::Document doc;
  ASSERT_NO_THROW(to_json_document(dist, doc));
  ASSERT_TRUE(doc.HasMember("cumulative"));
  ASSERT_TRUE(doc["cumulative"].IsBool());
  EXPECT_FALSE(doc["cumulative"].GetBool());
}

TEST(JsonSerialization, OutputDistributionWithoutCumulative)
{
  cryptonote::rpc::output_distribution dist{};
  dist.data.distribution = {1, 5, 14, 30, 55};
  dist.data.start_height = 2000000;
  ...
  // A daemon that predates `cumulative` emits every other key; parsing its
  // payload must succeed and leave the flag at its default.
  rapidjson::Document doc;
  ASSERT_NO_THROW(to_json_document(dist, doc));
  ASSERT_TRUE(doc.RemoveMember("cumulative"));
  ASSERT_FALSE(doc.HasMember("cumulative"));

  // Pre-set the flag so that a false result can only come from the reader
  // clearing it, never from the destination's initial value.
  cryptonote::rpc::output_distribution dist_copy{};
  dist_copy.cumulative = true;
  ASSERT_NO_THROW(cryptonote::json::fromJsonValue(doc, dist_copy));
