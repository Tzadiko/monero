...
demonstrated. Xcode 15 ships a compiler based on LLVM 16, which makes it the
Apple equivalent of the Clang 16 floor, but no Apple compiler was available
when the matrix was measured: Xcode 15 itself was never configured, built or
tested, and the `build-macos` CI job — the only continuous evidence — runs the
newer Xcode that `macOS-latest` ships. One pinned Xcode 15 configure, build and
test run is
required before the floor is published; should Xcode 15 not pass, the floor is
raised to the oldest Xcode that does, in this matrix, in `README.md` and in the
guard together.
...
The guard checks compiler versions only. It therefore cannot detect the Clang
16 with libstdc++ 14 pairing defect, which is a header-library mismatch rather
than a compiler version, and that is exactly why the matrix carries that row:
...
deprecated `std::aligned_storage` emits one deprecation diagnostic, reached
